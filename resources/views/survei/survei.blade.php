...
<body>
    <div class="survey-wrapper">
        <div class="card-custom"
            style="max-width: 700px; width: 100%; background: white; border-radius: 15px; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
            ...
            <div class="survey-body" style="padding: 30px;">
                @if($antreanSurvei)
                    <div class="info-vehicle">
                        ...
                        <div>
                            <span style="font-size: 11px; color: #6b7280; font-weight: bold; display: block;">NOMOR
                                KENDARAAN:</span>
                            <span
                                style="font-size: 20px; font-weight: 700; color: #1e3a8a;">{{ $antreanSurvei->kendaraan->no_kendaraan }}</span>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('survei.store') }}" method="POST">
                        ...
                        @foreach($listPos as $key => $label)
                            <div class="pos-rating-item">
                                <span class="pos-label"><i class="fa-solid fa-location-dot mr-2" style="color: #3b82f6;"></i>
                                    {{ $label }}</span>
                                <div class="rating-stars">
                                    @for($i = 5; $i >= 1; $i--)
                                        <input type="radio" name="ratings[{{ $key }}][skor]" id="{{ $key }}_{{ $i }}"
                                            value="{{ $i }}" required>
                        ...
                        <div style="margin-top: 25px;">
                            <label style="display:block; margin-bottom: 10px; font-weight: 600; color: #374151;">Saran &
                                Kritik Keseluruhan:</label>
                        ...
                        <button type="submit" class="btn-primary-custom"
                            style="width: 100%; padding: 15px; background: #1e3a8a; color: white; border: none; border-radius: 12px; font-weight: 700; font-size: 16px; cursor: pointer; margin-top: 20px; transition: 0.3s;">
                ...
                @else
                    <div style="text-align: center; padding: 40px 0;">
                        <i class="fa-solid fa-circle-check"
                            style="font-size: 80px; color: #10b981; margin-bottom: 20px;"></i>
